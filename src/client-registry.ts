...
export class _InternalRateLimiterRegistry<TAllNames extends string> { // Made generic
   ...
   */
  public getRedisClientKey(config?: { // Changed to public
    redis?: RedisOptions;
    envRedisKey?: string;
  }): string {
    ...
    try {
      const keyOptions = { host: effectiveOptions.host, port: effectiveOptions.port, database: effectiveOptions.database, username: effectiveOptions.username };
      return `opts_obj:${JSON.stringify(keyOptions)}`;
    } catch (e) { return `opts_fallback:${Date.now()}_${Math.random()}`; }
  }

  // --- getManagedRedisClient remains private ---
  private getManagedRedisClient(config?: { redis?: RedisOptions; envRedisKey?: string }): Promise<RedisClientType> {
    // Pass effective options when calling getRedisClientKey internally
     ...
     const clientKey = this.getRedisClientKey({redis: effectiveOptions, envRedisKey: config?.envRedisKey});

    if (!this.redisClients[clientKey]) {
       console.log(`[_InternalRateLimiterRegistry] Creating new Redis client promise for key: ${clientKey}`);
       ...
       this.redisClients[clientKey] = getRedisClient(finalFactoryOptions)
            .then(client => { /* ...log, emit */ return client; })
            .catch(err => { /* ...log, delete, emit */ throw err; });
    }
  ...
  public register( // Removed <T extends string> here
    ...
    if (typeof name !== "string" || !name.trim()) { // Keep runtime check
      return Promise.reject(new Error("[_InternalRateLimiterRegistry] Registration name must be a non-empty string."));
    }
    ...
    const initializationPromise = (async (): Promise<Ratelimit> => {
      try {
        ...
        console.log(`[_InternalRateLimiterRegistry] Registered limiter "${String(name)}" using Redis client key: ${clientKey}`);
        this.registryEmitter.emit("limiterRegister", { name: name, clientKey });
        this.limiters[name] = instance;
        return instance;
      } catch (error) {
        console.error(`[_InternalRateLimiterRegistry] Failed to initialize limiter "${String(name)}":`, error);
  ...
  // Use the generic TAllNames for the name parameter type
  public get(name: TAllNames): Ratelimit { // Removed <T extends string>
     const instance = this.limiters[name];
     if (!instance) { throw new Error(`[_InternalRateLimiterRegistry] Limiter "${String(name)}" not found.`); }
     if (instance instanceof Promise) { throw new Error(`[_InternalRateLimiterRegistry] Limiter "${String(name)}" still initializing.`); }
  ...
  public async close(): Promise<void> {
    // ... (close logic remains the same, using client.quit()) ...
     ...
     console.log(`[_InternalRateLimiterRegistry] Closing ${clientKeys.length} managed Redis client(s)...`);
     const quitPromises: Promise<void>[] = [];
     for (const key of clientKeys) {
         const clientPromise = this.redisClients[key];
         const quitPromise = clientPromise
             .then(client => client && typeof client.quit === 'function' ? client.quit().then(()=>{
              // not returning client
             }) : Promise.resolve())
             .catch(err => { console.error(`Error quitting client ${key}:`, err); return Promise.resolve(); });
...
public  on( eventName: string, listener: (...args: any[]) => void): this { this.registryEmitter.on(eventName, listener); return this; }
  // biome-ignore lint/suspicious/noExplicitAny: <explanation>
public  off( eventName: string, listener: (...args: any[]) => void): this { this.registryEmitter.off(eventName, listener); return this; }
}
...
export interface RateLimitBuilder<TNames extends string> { // Made generic
    ...
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
on: ( eventName: 'redisConnect' | 'redisError' | 'limiterRegister' | 'limiterError' | 'close', listener: (...args: any[]) => void ) => RateLimitBuilder<TNames>; // Returns typed builder
    /** Allows removing registry event listeners */
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
off: ( eventName: 'redisConnect' | 'redisError' | 'limiterRegister' | 'limiterError' | 'close', listener: (...args: any[]) => void ) => RateLimitBuilder<TNames>; // Returns typed builder
    /** Attempts to get the managed Redis client promise based on configuration key */
    getClient: (config: { redis?: RedisOptions; envRedisKey?: string }) => Promise<RedisClientType> | undefined;
}
