import { RedisClientType, createClient } from 'redis'; // Import RedisClientType if needed for casting
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
// Import RateLimitExceededError if you uncomment the block test
import { RateLimitExceededError, RatelimitError } from '../src';
...
describe('Ratelimit', () => {
  ...
  describe('token bucket limiter', () => {
    ...
    it('should block requests when no tokens are available', async () => {
      ...
      redis.eval.mockResolvedValueOnce([
        ...
        limit,
        0,
        MOCK_TB_TIME + interval / refillRate,
        retryAfter,
      ]); // Mock failure
  ...
  describe('block', () => {
    ...
    beforeEach(() => {
      ...
      vi.spyOn(Date, 'now').mockImplementation(
        () => vi.getMockedSystemTime()?.valueOf() ?? Date.now()
      );
    });
    ...
    it('should wait and retry when rate limited', async () => {
      ...
      expect(redis.eval).toHaveBeenNthCalledWith(
        ...
        expect.anything(),
        expect.objectContaining({
          keys: ['test-block:user-block-wait', 'test-block:user-block-wait:analytics'],
        })
      );
  ...
  describe('reset', () => {
    ...
    it('should use SCAN and delete keys in a single call for fixed window', async () => {
      ...
      expect(redis.del).toHaveBeenCalledWith(
        expect.arrayContaining([baseKey, analyticsKey, windowKey1, windowKey2])
      );
      expect(redis.del).toHaveBeenCalledWith([
        // Check exact array
        baseKey,
